...
def initialize_firestore():
    """Initialize Firestore with explicit project configuration"""
    ...
    try:
        if not firebase_admin._apps:
            ...
            if service_account_path and os.path.exists(service_account_path):
                print(f"🔥 Initializing Firebase with service account and project ID: {project_id}")
                cred = credentials.Certificate(service_account_path)
                firebase_admin.initialize_app(cred, {
                    'projectId': project_id
                })
            else:
                print(f"🔥 Initializing Firebase with default credentials and project ID: {project_id}")
